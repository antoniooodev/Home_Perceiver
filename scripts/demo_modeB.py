    ...
    for idx, (xy, conf, cls) in enumerate(zip(boxes_xy, confs, classes)):
        ...
        if cls == 0 and masks_data is not None:
            ...
            full_mask = cv2.resize(
                mask_small, (Wf, Hf), interpolation=cv2.INTER_NEAREST
            )
            ...
            cv2.putText(
                frame, label, (tx, ty), font, scale, (0, 0, 0), thickness, cv2.LINE_AA
            )
            ...
            ty = max(y1 - 10, th + 5)
            cv2.rectangle(
                frame, (x1, ty - th - 3), (x1 + tw + 3, ty + 3), color, cv2.FILLED
            )
            cv2.putText(
                frame,
                txt,
