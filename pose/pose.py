...
def draw_skeleton(
    frame, poses, kp_radius=3, kp_color=(0, 0, 255), line_color=(255, 0, 0), thickness=2
):
    """
    ...
    for person in poses:
        ...
        if v0 > 0 and v5 > 0 and v6 > 0:
            ...
            cv2.line(
                frame, (int(x0), int(y0)), (int(mx), int(my)), line_color, thickness
            )
        ...
        for i, j in SKELETON:
            ...
            if v1 > 0 and v2 > 0:
                cv2.line(
                    frame, (int(x1), int(y1)), (int(x2), int(y2)), line_color, thickness
                )


def run_pose_on_frame(model, frame, min_score=0.5):
